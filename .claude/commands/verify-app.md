...
2-1. **앱소싱 데이터 탐색** — `build_app_context.json`의 `sourcing_data` 경로가 있으면:
   - `앱_설명_모음.md` → 경쟁사 앱 UI/UX 특성 파악 (검증 시 비교 기준)
   - `앱소싱 리포트*.md` → 리뷰 불만 패턴 → 고도화 모드(Step F)에서 경쟁사 약점 보완 우선순위
   - 소싱 데이터는 **검증 판정 기준을 풍부하게** 하는 용도:
     - 경쟁사에서 공통적으로 지원하는 기능이 누락되면 P1으로 격상
     - 리뷰에서 가장 불만이 많은 카테고리(버그/UX)와 관련된 화면은 검증 우선순위 상향

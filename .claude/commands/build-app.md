...
이 스킬은 아래 형식의 MVP md 파일을 입력으로 받는다. 파싱 시 각 섹션 번호와 헤더를 기준으로 추출한다.
...
1. MVP md 파일을 읽고 다음을 `/tmp/build_app_context.json`으로 추출 저장:
   ```json
   {
...
5. 보일러플레이트의 기존 구조 확인: `lib/core/`, `lib/features/`, `pubspec.yaml`

6. Tier에 따라 Phase 1 에이전트 팀을 실행한다.
...
- lib/features/ 기존 화면 구조 참고

작업:
1. 섹션 8 화면 목록 기준으로 screens/ 생성 (온보딩/페이월 제외)
2. go_router 라우트 설정 (routes.dart)
3. Riverpod providers — agent2_manifest.json의 실제 클래스명/메서드 사용
4. 공유 위젯 (달력, 차트, 커스텀 버튼 등)
5. 온보딩 완료 후 메인 화면으로의 라우팅 설정 (온보딩 화면 자체는 Agent 4가 담당)
6. 무료/유료 기능 분기 UI
7. /tmp/required_packages_agent3.txt 생성
```
...
3. 리워드 광고 보상 로직 (있으면)
4. 테스트 광고 유닛 ID 설정
5. docs/AD_UNIT_IDS.md 프로덕션 교체 가이드
...
작업:
1. docs/STORE_LISTING.md — 스토어 등록 정보 (섹션 1 메타데이터 그대로)
2. docs/ASO_KEYWORDS.md — 키워드 우선순위 + 배치 전략
3. docs/MARKETING_PLAN.md — Stage 1~4 체크리스트
4. docs/CRM_SEQUENCES.md — 온보딩/리텐션/이탈방지 시퀀스
...
에이전트 구조 밖에서 개발자가 수행해야 하는 비코드 작업 체크리스트:
- [ ] 앱 서명 키 생성 (Keystore / Certificate)
- [ ] Google Play Console 앱 등록
- [ ] App Store Connect 앱 등록
- [ ] RevenueCat 콘솔 설정 (docs/REVENUECAT_SETUP.md 참조)
- [ ] AdMob 프로덕션 광고 유닛 ID 교체 (docs/AD_UNIT_IDS.md 참조)
- [ ] Firebase 프로젝트 생성 + google-services.json / GoogleService-Info.plist 배치
- [ ] ASO 스토어 등록 정보 입력 (docs/STORE_LISTING.md 참조)
- [ ] 스크린샷 5종 준비
- [ ] 개인정보 처리방침 URL 준비
